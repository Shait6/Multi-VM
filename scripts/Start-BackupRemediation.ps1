...
if (-not $existingDefId) {
  if (Test-Path $policyRulesPath) {
    az policy definition create --name $CustomPolicyDefinitionName --display-name "Central VM Backup" --description "Assign central backup policy to tagged VMs." --rules $policyRulesPath --mode Indexed -o none
  } else {
